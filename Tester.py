...
def openJSONfile(filePath, fileName):
    with open(filePath + fileName + '.json') as f: 
        JSONdata = json.load(f)
        return JSONdata

la_en = openJSONfile(".//vocab/","la-en")
en = la_en["en"]
la = la_en["la"]
laen = dict(zip(la,en))
...
def main_menu():
    ...
    skip=Checkbutton(tkmenu,text="skip marking animation\nfor correct answers",variable=skipvar,font=("Calibri", 15))
    label_1.grid(row=0,column=0)
